...
public class MeasureTool implements OnGestureListener {

	private final UCMapView mMapView;
		...
		if (type == 0) {
			...
			if (points.size() == 1)
				...
				if (1 <= distance && distance < 1000)
					string = String.format("%.1f米", distance);
				else if (distance >= 1000)
					string = String.format("%.1f千米", distance / 1000);
				else if (0.01 <= distance && distance < 1)
					string = String.format("%.1f厘米", distance * 100);
				else
					string = String.format("%.1f毫米", distance * 1000);
			}
			...
			if (lastText != null)
				mlayer.removeItem(lastText);
			// 添加文字
			double area = getResult();
			String string = null;
			if (area < 100000)
				string = String.format("%.1f平方米", area);
			else
				string = String.format("%.1f平方公里", area / 1000 / 1000);
			int nn = points.size();
			double x = 0, y = 0;
			for (int i = 0; i < nn; ++i) {
				x += points.get(i).getX();
